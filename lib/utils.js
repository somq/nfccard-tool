...
var utils = {
    ...
    /**
     * @description Fills a buffer until % of an int equals zero with a value
     * @returns a filled buffer
     */
    bufferFiller: function(initialBuffer, moduloNumber, filler) {
      let i=0;
      let initBuffLen = initialBuffer.length;
      while(initBuffLen % 4 !== 0) {
        i++;
        initBuffLen++;
      }
      const bufferFiller = Buffer.allocUnsafe(i)
      bufferFiller.fill(0);

      return Buffer.concat([initialBuffer, bufferFiller], initialBuffer.length + bufferFiller.length);
    }

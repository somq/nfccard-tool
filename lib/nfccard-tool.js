  ...
  const tagHeaderValues = {
    ...
    string: {
      ...
      capabilityContainer: {
        ...
        MAX_NDEF_SIZE: utils.decimalToHexString(tagHeader[14]),
...
nfcCardTool.prototype.prepareBytesToWrite = function(message) {
  ...
  let preparationWriteToFill = Buffer.concat([NDEF_MESSAGE_TLV_T, NDEF_MESSAGE_TLV_L_LENGTH, NDEFMessageBuffer, TERMINATOR], NDEFMessage.toByteArray().length + 3) // 3 = NDEF_MESSAGE_TLV_T + NDEF_MESSAGE_TLV_L_EMPTY + TERMINATOR
  ...
  let preparationWrite = utils.bufferFiller(preparationWriteToFill, 4, 0)

  // NDEFMessageBuffer.length = 2;
  // console.log(NDEFMessageBuffer)
